...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  // арккосинус для получения угла в радианах
...
function roundToPowerOfTen(num, pow) {
  const powDegree = 10 ** pow;
  return Math.round(num / powDegree) * powDegree;
}
...
function toNumber(value, def) {
  ...
  if (!Number.isNaN(convertValue) && typeof convertValue === 'number') {
    return value;
  }
  return def;
}
...
function getIntegerOnString(str, base) {
  // const floatingStr = Number.parseFloat(str);
  // const baseFloatingStr = floatingStr.toString(base);
